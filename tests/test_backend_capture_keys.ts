...
function fixture() {
  ...
  const state = (paddedKvLen: number, totalTokens = 4, sparseMode = true, contextParallel = true) => ({
    ...
    cache: { getPagedKV: () => ({ sparseMode, contextParallel, glm: ops }) },
...
test("CP capture keys distinguish grouping changes and reuse saturated buckets", () => {
  ...
  assert.deepEqual(ops.getCaptureKeys(state(16384)), ops.getCaptureKeys(state(65536)));
  assert.deepEqual(ops.getCaptureKeys(state(4096, 1)), ops.getCaptureKeys(state(8192, 1)));
  const s = state(4096);
  ops.getCaptureKeys(s);
  assert.equal(s.paddedKvLenInvariant, true);
  assert.deepEqual(ops.getCaptureKeys(state(4096, 4, false)), []);
  assert.deepEqual(ops.getCaptureKeys(state(4096, 4, true, false)), []);
  assert.deepEqual(ops.getCaptureKeys(state(4096, 17)), []);
});
...
test("capture lookup incorporates each state's backend launch keys", () => {
  ...
  assert.deepEqual(lookup(16384), lookup(32768));
...
test("device capture keys own dispatch and ParallelOps preserves each device's decision", () => {
  const { ops, state } = fixture();
  const s = state(16384);
  assert.deepEqual(ops.devices[0].getCaptureKeys(s), ["sparseMlaChunksPerBlock:6"]);
  Object.assign(ops.devices[1], { smCount: 188 });
  const keys = ops.getCaptureKeys(s);
  assert.equal(keys[0], "device:0:sparseMlaChunksPerBlock:6");
  assert.equal(keys[1], "device:1:sparseMlaChunksPerBlock:3");
  assert.deepEqual(ops.devices[0].getCaptureKeys(state(16384, 17)), []);
  const decode = Object.assign(state(16384, 17), { isDecode: true });
  assert.deepEqual(ops.devices[0].getCaptureKeys(decode), ["sparseMlaChunksPerBlock:2"]);
});
