...
test("CP capture keys distinguish grouping changes and reuse saturated buckets", () => {
  ...
  assert.deepEqual(ops.devices[0].getCaptureKeys(state(4096, 4, true, false)), []);
  assert.deepEqual(ops.devices[0].getCaptureKeys(state(4096, 17)), []);
});
