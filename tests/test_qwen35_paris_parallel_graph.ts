...
describe("Qwen3.5-0.8B generate Paris with CUDA graphs (2-GPU ParallelOps)", () => {
  let glm0: GlmOps;
  let glm1: GlmOps;
  let po: ParallelOps;
  ...
  before(async () => {
    ...
    model = Qwen35Model.fromPretrained(po as any, QWEN35_REPO, 1, 128);
  ...
  it("generates 'Paris' using CUDA graph decode (2 GPUs)", () => {
