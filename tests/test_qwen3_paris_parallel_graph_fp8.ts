...
describe("Qwen3-0.6B-FP8 generate Paris with CUDA graphs (2-GPU ParallelOps)", () => {
  let glm0: GlmOps;
  let glm1: GlmOps;
  let po: ParallelOps;
  ...
  before(async () => {
    ...
    model = Qwen3Model.fromPretrained(po as any, FP8_REPO, 1, 128);
    ...
    const modelDir = resolveModelPath(FP8_REPO);
  ...
  it("generates 'Paris' using FP8 + CUDA graph decode (2 GPUs)", () => {
    ...
    let warmupRemaining = warmupSteps;
    let capturing = false;
    ...
    for (let i = 1; i < maxNewTokens && !eosIds.has(currentToken); i++) {
      ...
      if (graphExec !== null) {
        po.graphLaunch(graphExec);
        po.synchronize();
      } else {
        if (warmupRemaining === 0 && !capturing) {
          capturing = true;
          po.graphBeginCapture();
        }
