...
class TestBf16MulMatIdGroupedMMA:
    ...
    def test_expert_imbalance(self, glm, device):
        """All tokens routed to expert 0 — exercises M-tile splitting across
        many M-tiles for one expert while other experts are empty."""
        batch = 32
        K = 256
        N = 128
        num_experts = 8
    ...
    def test_heavy_imbalance(self, glm, device):
        """Most tokens to expert 0, one token each to experts 1-3."""
        batch = 20
        K = 128
        N = 64
        num_experts = 4
