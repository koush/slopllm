...
def test_kv_cache_write_prefill_layout(glm):
    ...
    src_v = np.random.randn(n_kv, num_tokens, hd).astype(np.float32)
    ...
    src_v_gpu = glm.alloc(n_kv * num_tokens * hd * BF16)
    dst_k_gpu = glm.alloc(max_pages * n_kv * page_size * hd * BF16)
    dst_v_gpu = glm.alloc(max_pages * n_kv * page_size * hd * BF16)
    slot_gpu = glm.alloc(num_tokens * I32)
    ...
    for t, slot in enumerate(slot_mapping):
        ...
        for h in range(n_kv):
            ...
            np.testing.assert_allclose(result_v[page, h, slot_in_page], src_v[h, t], atol=1e-2, rtol=1e-2)
...
def test_kv_cache_write_decode_and_prefill_produce_same_result(glm):
    ...
    src_prefill_v = data_v.transpose(1, 0, 2).copy()
    ...
    src_prefill_v_gpu = glm.alloc(n_kv * num_tokens * hd * BF16)
