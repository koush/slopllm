...
from test_attention_absorbed import attention_forward_absorbed_cuda, attention_forward_absorbed_torch
from test_moe import moe_forward_torch, moe_forward_cuda
from test_decoder_layer import decoder_layer_torch


def decoder_layer_absorbed_torch(hidden_states, cos, sin, attention_mask,
    ...
    attn_out, q_resid = attention_forward_absorbed_torch(
        h, cos, sin, attention_mask,
        attn_weights['q_a_proj_w'], attn_weights['q_a_layernorm_w'],
        attn_weights['q_b_proj_w'],
        attn_weights['kv_a_proj_with_mqa_w'], attn_weights['kv_a_layernorm_w'],
        attn_weights['kv_b_proj_w'],
        attn_weights['o_proj_w'],
        attn_weights['indexer_weights'],
        attn_weights['num_heads'], attn_weights['qk_nope_dim'],
        attn_weights['qk_rope_dim'], attn_weights['v_head_dim'],
        attn_weights['q_lora_rank'], attn_weights['kv_lora_rank'],
        attn_weights['hidden_size'], eps)
    ...
    if mlp_type == "dense":
        gate = torch.nn.functional.linear(h, mlp_weights['gate_w'])
        up = torch.nn.functional.linear(h, mlp_weights['up_w'])
        gate_up = (torch.nn.functional.silu(gate.float()) * up.float()).to(torch.bfloat16)
        mlp_out = torch.nn.functional.linear(gate_up, mlp_weights['down_w'])
    else:
...
def decoder_layer_absorbed_cuda(glm, device, hidden_states, cos, sin, attention_mask,
                                input_layernorm_w, post_attn_layernorm_w,
                                attn_weights, mlp_weights, mlp_type, eps=1e-5):
    ...
    attn_out, q_resid = attention_forward_absorbed_cuda(
        glm, device, h, cos, sin, attention_mask,
        attn_weights['q_a_proj_w'], attn_weights['q_a_layernorm_w'],
        attn_weights['q_b_proj_w'],
        attn_weights['kv_a_proj_with_mqa_w'], attn_weights['kv_a_layernorm_w'],
        attn_weights['kv_b_proj_w'],
    ...
    if mlp_type == "dense":
        gate_w = mlp_weights['gate_w']
        up_w = mlp_weights['up_w']
        down_w = mlp_weights['down_w']
        ...
        glm.silu_and_mul(gate_up.reshape(-1), gate.reshape(-1), up.reshape(-1),
                         intermediate, B * S)
        mlp_out = torch.empty(B * S, hidden, dtype=torch.bfloat16, device=device)
        glm.linear(mlp_out, gate_up, down_w, B * S, hidden, intermediate)
        mlp_out = mlp_out.reshape(B, S, hidden)
    else:
...
def test_decoder_layer_absorbed_dense(glm, device):
    B, S = 1, 4
    hidden = 64
    num_heads = 2
    qk_nope_dim = 12
    ...
    ref = decoder_layer_absorbed_torch(
        hidden_states, cos, sin, attention_mask,
        input_layernorm_w, post_attn_layernorm_w,
        attn_weights, mlp_weights, "dense", eps)

    cuda = decoder_layer_absorbed_cuda(
        glm, device, hidden_states, cos, sin, attention_mask,
        input_layernorm_w, post_attn_layernorm_w,
        attn_weights, mlp_weights, "dense", eps)

    torch.testing.assert_close(cuda.cpu(), ref.cpu(), atol=20.0, rtol=5e-1)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_decoder_layer_absorbed_no_mask_dense(glm, device):
    B, S = 2, 4
    hidden = 64
    num_heads = 2
    qk_nope_dim = 12
    ...
    exp = decoder_layer_torch(
        hidden_states, cos, sin, None,
        input_layernorm_w, post_attn_layernorm_w,
        attn_weights, mlp_weights, "dense", eps)
    torch.testing.assert_close(cuda.cpu(), exp.cpu(), atol=20.0, rtol=5e-1)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_decoder_layer_absorbed_moe(glm, device):
    B, S = 1, 4
    hidden = 32
    num_heads = 2
    qk_nope_dim = 8
