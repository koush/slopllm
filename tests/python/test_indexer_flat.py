...
def _make_caches(device, seq_lens, page_size, head_dim, seed=42):
    ...
    paged, paged_scales = pack_indexer_k(paged_bf16)
    ...
    for length, page_count in zip(seq_lens, page_counts):
        ...
        scale_parts = [paged_scales[page_id, :last if i == page_count - 1 else page_size]
                       for i, page_id in enumerate(ids)]
        flat_scale_parts.append(torch.cat(scale_parts, dim=0))
    ...
    return {
        "paged": paged,
        "paged_scales": paged_scales,
        "flat": torch.cat(flat_parts, dim=0).contiguous(),
        "flat_scales": torch.cat(flat_scale_parts, dim=0).contiguous(),
...
def test_flat_append_multi_sequence(glm, device, head_dim):
    ...
    destination = torch.full((sum(seq_lens), head_dim), 0xa5, dtype=torch.uint8, device=device)
    destination_scales = torch.full((sum(seq_lens),), -1, dtype=torch.float32, device=device)
    expected = destination.clone()
    expected_scales = destination_scales.clone()

    for i, (batch, position) in enumerate(zip(batch_indices.tolist(), positions.tolist())):
        packed, scales = pack_indexer_k(source_storage[i:i + 1, :head_dim])
        slot = kv_token_indptr[batch].item() + position
        expected[slot] = packed[0]
        expected_scales[slot] = scales[0]

    glm.indexer_kv_cache_append_flat(
        destination, destination_scales, source_storage, kv_token_indptr, batch_indices, positions,
        len(batch_indices), head_dim, stride,
    )
    glm.synchronize()

    assert torch.equal(destination, expected)
    assert torch.equal(destination_scales, expected_scales)
...
def test_paged_append_cp_packs_owned_tokens(glm, device):
    ...
    destination = torch.zeros(1, page_size // world_size, head_dim,
                              dtype=torch.uint8, device=device)
    destination_scales = torch.zeros(1, page_size // world_size,
    ...
    glm.mla_kv_cache_append(
        destination.data_ptr(), destination_scales.data_ptr(),
    ...
    expected, expected_scales = pack_indexer_k(append_k[rank::world_size])
    assert torch.equal(destination[0], expected)
    assert torch.equal(destination_scales[0], expected_scales)
...
def test_flat_append_patches_gathered_pages(glm, device):
    ...
    gathered = torch.empty(sum(seq_lens), head_dim, dtype=torch.uint8, device=device)
    gathered_scales = torch.empty(sum(seq_lens), dtype=torch.float32, device=device)
    glm.gather_pages(
    ...
    glm.gather_pages(
        gathered_scales, caches["paged_scales"], caches["page_indices"], caches["page_indptr"],
        caches["last_page_len"], caches["paged_scales"].shape[0], len(seq_lens),
        page_size, 1,
    )
    ...
    expected_scales = caches["flat_scales"].clone()
    for i, (batch, position) in enumerate(zip(batch_indices.tolist(), positions.tolist())):
        packed, scales = pack_indexer_k(append_k[i:i + 1])
        slot = caches["kv_token_indptr"][batch].item() + position
        expected[slot] = packed[0]
        expected_scales[slot] = scales[0]

    glm.indexer_kv_cache_append_flat(
        gathered, gathered_scales, append_k, caches["kv_token_indptr"], batch_indices, positions,
        len(batch_indices), head_dim, head_dim,
    )
    glm.synchronize()

    assert torch.equal(gathered, expected)
    assert torch.equal(gathered_scales, expected_scales)
...
def test_flat_raw_score_matches_paged(glm, device, causal):
    ...
    args = (
        q, caches["paged"], caches["paged_scales"], weights,
        caches["page_indices"], caches["page_indptr"],
    ...
    flat_args[2] = caches["flat_scales"]
...
def _run_topk(glm, mode, q, k_data, k_scale_data, weights, caches, qo_indptr, topk, max_kv,
    ...
    if mode == "v2":
        ...
        glm.indexer_score_topk_v2(
            out, out_scores, q, k_data, k_scale_data, weights,
...
def test_flat_topk_matches_paged(glm, device, mode):
    ...
    paged = _run_topk(glm, mode, q, caches["paged"], caches["paged_scales"],
                      weights, caches, qo_indptr,
                      topk, max_kv, True)
    flat = _run_topk(glm, mode, q, caches["flat"], caches["flat_scales"],
                     weights, caches, qo_indptr,
