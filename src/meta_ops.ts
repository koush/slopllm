...
export class MetaTensor extends Tensor {
    ...
    bmm(B: Tensor, batch: number, M: number, N: number, K: number, transA: boolean = false, transB: boolean = false, tokenMajor: boolean = false): Tensor {
        ...
        return this.workspace.alloc(tokenMajor ? [M, batch, N] : [batch * M, N], this.type);
    }
...
export class MetaOps implements DeviceOps {
    getCaptureKeys(_state: ExecutionState): readonly (string | number)[] {
        return [];
    }
    ...
    indexerQuantizeQ(q: Tensor, _weights: Tensor, _scale: number, _weightHeadOffset?: number): { q8: Tensor, effectiveWeights: Tensor | undefined } {
        // Mirrors the production eligibility rule: FP8 quantization only exists
        // for the 32-head / 128-dim query (and can be disabled by env). Meta
        // builds return the identity fallback otherwise.
