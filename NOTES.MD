...
using _tracker = ws.startTracking();
{
  ...
  using gateBuf = normed.linear(weights.gate_proj, BS);  // disposed when this block exits
  using upBuf = normed.linear(weights.up_proj, BS);      // disposed when this block exits
